...
# Search for emails from today with specific subject
status, selected_mails = mail.search(None, f'(ON "{today_date}" FROM "reports@cloudcardinc.com" SUBJECT "Database Storage Statistic of")')
...
print(f"Total Messages from reports@cloudcardinc.com on {today_date} with subject 'Database Storage Statistic of':", len(email_ids))
...
if len(email_ids) == 0:
        ...
        try:
            ...
            for part in email_message.walk():
                if part.get_content_type() == "text/plain" or part.get_content_type() == "text/html":
                    ...
                    db_stats = re.findall(r'Below are the database storage statistics of (\S+) DB.*?Size in GB = (\d+\.\d+)', message, re.DOTALL)
    ...
    if len(sheet.row_values(1)) == 0:
        headers = ["DB Name", "Size in GB values", "Date"]
        sheet.append_row(headers)

    # Append data to Google Sheets
    last_col = get_last_col(sheet)
    date = datetime.now().strftime('%Y-%m-%d')
    for db_name, size_in_gb in zip(db_names, sizes_in_gb):
        row = [db_name, size_in_gb, date]
